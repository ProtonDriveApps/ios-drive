...
public final class MacOSPausableTimerResource: PausableTimerResource {
    ...
    private func handleIntervalEnd() {
        ...
        if !hasStopBeenRequested {
            // resume schedules another tick of the timer, which should happen if there were no request for stop
            resume()
            // the caller may stop the timer in the receive closure of subscription, this will set hasStopBeenRequested = true and prevent next schedule
            subject.send()
        }
    }
}
