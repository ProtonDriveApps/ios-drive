...
public enum SDKNodeOperationStreamEvent: Sendable {
    case nodeResults(
        results: [(AnyVolumeIdentifier, error: Error?)],
        affectedIdentifiers: [AnyVolumeIdentifier]
    )
    case completed(error: Error?)
}

extension AsyncThrowingStream where Element == SDKNodeOperationStreamEvent {
