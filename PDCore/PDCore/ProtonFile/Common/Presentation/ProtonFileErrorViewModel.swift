...
public protocol ProtonFileErrorViewModelProtocol {
    func handleError(_ error: Error)
}

public final class ProtonFileErrorViewModel: ProtonFileErrorViewModelProtocol {
    ...
    public func handleError(_ error: Error) {
        let message = makeMessage(from: error as? ProtonFileOpeningError)
        let plainError = PlainMessageError(message)
        messageHandler.handleError(plainError)
    }

    private func makeMessage(from error: ProtonFileOpeningError?) -> String {
        switch error {
