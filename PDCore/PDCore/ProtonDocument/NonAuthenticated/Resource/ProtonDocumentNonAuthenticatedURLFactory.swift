...
public protocol ProtonDocumentNonAuthenticatedURLFactoryProtocol {
    func makeURL(from identifier: ProtonDocumentIdentifier) throws -> URL
}

enum ProtonDocumentNonAuthenticatedURLFactoryError: Error {
    case invalidHost
    case invalidURL
}

final class ProtonDocumentNonAuthenticatedURLFactory: ProtonDocumentNonAuthenticatedURLFactoryProtocol {
    ...
    func makeURL(from identifier: ProtonDocumentIdentifier) throws -> URL {
        guard var urlComponents = URLComponents(string: configuration.baseOrigin) else {
            throw ProtonDocumentNonAuthenticatedURLFactoryError.invalidHost
        }
        ...
        guard let url = urlComponents.url else {
            throw ProtonDocumentNonAuthenticatedURLFactoryError.invalidURL
        }
        return url
    }
}
