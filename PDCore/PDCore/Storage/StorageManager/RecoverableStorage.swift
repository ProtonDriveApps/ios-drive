...
/// Cross-process coordination flags for the full resync lifecycle.
/// Prevents the FileProvider extension from interfering with the main app's recovery DB,
/// and signals the extension to reload its persistent store after the DB is replaced.
public final class RecoveryCoordination {
    @SettingsStorage("fullResyncRecoveryInProgress", suite: .group(named: Constants.appGroup))
    private static var inProgress: Bool?
    @SettingsStorage("fullResyncStoreWasReplaced", suite: .group(named: Constants.appGroup))
    private static var storeReplaced: Bool?
    ...
    public static func clearInProgress() {
        inProgress = nil
    }
    ...
    public static func markStoreReplaced() {
        storeReplaced = true
    }

    /// Returns true if the store was replaced, consuming the flag.
    public static func consumeStoreReplaced() -> Bool {
        guard storeReplaced ?? false else { return false }
        storeReplaced = nil
        return true
    }
}
