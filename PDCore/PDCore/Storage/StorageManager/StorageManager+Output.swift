...
public extension StorageManager {
    ...
    func fetchRevision(id: RevisionIdentifier, moc: NSManagedObjectContext) -> Revision? {
        ...
        moc.performAndWait {
            ...
            let idPredicate = NSPredicate(format: "%K == %@", #keyPath(Revision.id), id.revisionID)
...
extension StorageManager {
    public func fetchActivePhotosCount(moc: NSManagedObjectContext) -> Int {
        return moc.performAndWait {
            guard let volumeId = getPhotosVolumeId(in: moc) else { return -1 }
            let fetchRequest = NSFetchRequest<Photo>(entityName: "Photo")
            fetchRequest.sortDescriptors = [NSSortDescriptor(key: #keyPath(Photo.captureTime), ascending: false)]
            fetchRequest.predicate = NSPredicate(
                format: "%K == %d AND %K == %@",
                #keyPath(Photo.stateRaw), Node.State.active.rawValue,
    ...
    public func fetchMyPrimaryUploadingPhotos(moc: NSManagedObjectContext) -> [Photo] {
        moc.performAndWait {
            let fetchRequest = requestMyPrimaryUploadingPhotos(moc: moc)
            return (try? moc.fetch(fetchRequest)) ?? []
        }
    }

    /// Fetches only the uploading photos where `nameSignatureEmail` is nil on the photo itself
    /// or on any of its children, avoiding faulting all uploading photos into memory.
    public func fetchCorruptedUploadingPhotos(moc: NSManagedObjectContext) -> [Photo] {
        moc.performAndWait {
            let basePredicate = makeMyPrimaryUploadingPhotoPredicate(moc: moc)

            let selfCorrupted = NSPredicate(format: "%K == nil", #keyPath(Photo.nameSignatureEmail))
            let childCorrupted = NSPredicate(
                format: "SUBQUERY(%K, $child, $child.nameSignatureEmail == nil).@count > 0",
                #keyPath(Photo.children)
            )
            let corruptedPredicate = NSCompoundPredicate(orPredicateWithSubpredicates: [selfCorrupted, childCorrupted])

            let fetchRequest = NSFetchRequest<Photo>(entityName: "Photo")
            fetchRequest.sortDescriptors = [NSSortDescriptor(key: #keyPath(Photo.captureTime), ascending: false)]
            fetchRequest.predicate = NSCompoundPredicate(andPredicateWithSubpredicates: [basePredicate, corruptedPredicate])

            return (try? moc.fetch(fetchRequest)) ?? []
        }
    }
    
    /// Fetches photos in recoverable upload states to be normalized during bootstrap.
    /// Includes both primary and child photos in the photos volume.
    public func fetchPhotosForInterruptedStateRecovery(moc: NSManagedObjectContext) -> [CoreDataPhoto] {
        moc.performAndWait {
            guard let volumeID = getPhotosVolumeId(in: moc) else { return [] }

            let recoverableStates: [NSNumber] = [
                NSNumber(value: Photo.State.uploading.rawValue),
                NSNumber(value: Photo.State.cloudImpediment.rawValue),
                NSNumber(value: Photo.State.paused.rawValue)
            ]

            let fetchRequest = NSFetchRequest<CoreDataPhoto>(entityName: "Photo")
            fetchRequest.sortDescriptors = [NSSortDescriptor(key: #keyPath(CoreDataPhoto.captureTime), ascending: false)]
            fetchRequest.predicate = NSPredicate(
                format: "%K == %@ AND %K IN %@",
                #keyPath(Photo.volumeID), volumeID,
                #keyPath(Photo.stateRaw), recoverableStates
            )
    ...
    /// For SDK what we care and use state is `interrupted`
    /// pause and cloudImpediment doesn't make sense for SDK
    func fetchUploadingPhotosForSDK(volumeId: String, size: Int, moc: NSManagedObjectContext) -> [Photo] {
        var fetchedPhotos: [Photo] = []
        moc.performAndWait {
            // Child photos that have their parents uploaded
            if let photos = try? moc.fetch(requestChildPhotosWithUploadedParentForSDK(volumeId: volumeId, size: size)), !photos.isEmpty {
                fetchedPhotos.append(contentsOf: photos)
            }
            guard fetchedPhotos.count < size else { return }
            ...
            // Calculate the remaining number of photos to fetch
            let remainingSize = size - fetchedPhotos.count
            guard remainingSize > 0 else { return } // Stop if we have fetched the desired number of photos

            if let photos = try? moc.fetch(requestPrimaryPhotos(volumeId: volumeId, ofState: .interrupted, size: remainingSize)) {
                fetchedPhotos.append(contentsOf: photos)
            }
        }
        return fetchedPhotos
    ...
    private func requestChildPhotosWithUploadedParentForSDK(volumeId: String, size: Int) -> NSFetchRequest<Photo> {
        let fetchRequest = NSFetchRequest<Photo>(entityName: "Photo")
        fetchRequest.sortDescriptors = [NSSortDescriptor(key: #keyPath(Photo.captureTime), ascending: false)]
        let hasParent = NSPredicate(format: "%K != nil", #keyPath(Photo.parent))
        let parentIsUploaded = NSPredicate(
    ...
    private func requestMyPrimaryUploadingPhotos(moc: NSManagedObjectContext, fetchPropertyValues: Bool = true) -> NSFetchRequest<Photo> {
        let fetchRequest = NSFetchRequest<Photo>(entityName: "Photo")
        fetchRequest.sortDescriptors = [NSSortDescriptor(key: #keyPath(Photo.captureTime), ascending: false)]
        fetchRequest.predicate = makeMyPrimaryUploadingPhotoPredicate(moc: moc)
    ...
    private func makeMyPrimaryUploadingPhotoPredicate(moc: NSManagedObjectContext) -> NSCompoundPredicate {
        let stateKeyPath = #keyPath(Photo.stateRaw)
        let childrenPath = #keyPath(Photo.children)
        let uploading = Photo.State.uploading.rawValue
        let cloudImpediment = Photo.State.cloudImpediment.rawValue
        let interrupted = Photo.State.interrupted.rawValue
        
        let uploadingStates = [uploading, cloudImpediment, interrupted]

        let selfStateMatch = NSPredicate(
            format: "%K IN %@", stateKeyPath, uploadingStates
        )
        let childStateMatch = NSPredicate(
            format: "SUBQUERY(%K, $child, $child.%K IN %@).@count > 0",
            childrenPath, stateKeyPath, uploadingStates
        )
        let statePredicate = NSCompoundPredicate(orPredicateWithSubpredicates: [selfStateMatch, childStateMatch])

        let isMainPhoto = NSPredicate(format: "%K == nil", #keyPath(Photo.parent))

        let volumeID = getPhotosVolumeId(in: moc) ?? ""
        if volumeID.isEmpty {
            Log.error("Photo volume is nil", error: nil, domain: .storage)
        }
        let myPhotoVolume = NSPredicate(format: "%K == %@", #keyPath(Photo.volumeID), volumeID)
        return NSCompoundPredicate(andPredicateWithSubpredicates: [myPhotoVolume, isMainPhoto, statePredicate])
    }
    ...
    public func subscriptionToMyPrimaryUploadingPhotosCount(moc: NSManagedObjectContext) -> NSFetchedResultsController<Photo> {
        return NSFetchedResultsController(
            fetchRequest: requestMyPrimaryUploadingPhotos(moc: moc, fetchPropertyValues: false),
            managedObjectContext: moc,
            sectionNameKeyPath: nil,
            cacheName: nil
        )
