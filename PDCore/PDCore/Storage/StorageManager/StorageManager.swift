...
public class StorageManager: NSObject, ManagedStorage, RecoverableStorage, RefreshableStorage {
    ...
    internal static func defaultPersistentContainer(suiteUrl: URL?) -> NSPersistentContainer {
        ...
        // SQLite tuning. The default page cache (~8MB) was leaving us with
        // page-cache misses on the upload hot path; a flamegraph showed >10%
        // of total samples in unixRead/seekAndRead under sqlite3_step.
        //
        // Positive values here mean the cache size is set to a number of pages,
        // negative means actual kilobytes so -131072 is 128MB (the default is 8MB).
        storeDescription.setOption(["cache_size": "-131072"] as NSDictionary, forKey: NSSQLitePragmasOption)
    ...
    public let userDefaults: UserDefaults // Ideally, this should be replaced with @SettingsStorage
    
    private static let recoveryDatabaseName = "Recovery_\(databaseName)"
    private static let backupDatabaseName = "Backup_\(databaseName)"
    private static let storeReloadLock = NSLock()
    ...
    public convenience init(suite: SettingsStorageSuite) {
        switch suite {
    ...
    public init(container: NSPersistentContainer, userDefaults: UserDefaults) {
        ...
        do {
            if Constants.runningInExtension && RecoveryCoordination.isInProgress {
        ...
        // initialize the context pools eagerly, as the lazy var is not thread safe
        _ = self.backgroundContextPool
        _ = self.synchronousContextPool
    }

    /// Checks if the main app replaced the DB during a full resync.
    /// If so, reloads the persistent store and resets all contexts so the extension picks up the new data.
    public func reloadStoreIfReplacedByMainApp() {
        ...
        do {
            ...
            try persistentContainer.persistentStoreCoordinator.addPersistentStore(type: NSPersistentStore.StoreType(rawValue: type), configuration: nil, at: storeURL)
            contexts.value.compactMap(\.reference).forEach { ctx in ctx.performAndWait { ctx.reset() } }
        } catch {
            Log.error("Failed to reload persistent store after DB replacement", error: error, domain: .storage)
        }
    }
    ...
    public lazy var synchronousContextPool: SyncManagedObjectContextPool = {
    ...
    public func cleanUp() async {
        ...
        await self.backgroundContext.perform {
            ...
            if self.mainContext.isInMemory {
