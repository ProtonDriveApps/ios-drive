...
public final class MigrationDetector {
    ...
    internal func checkIfRequiresPostMigrationCleanup(storeAt storeFileUrl: URL, for model: NSManagedObjectModel) {
        guard requiresPostMigrationCleanup != true else { return }
        requiresPostMigrationCleanup = coreDataWillInvokeMigration(storeAt: storeFileUrl, for: model)
    ...
    private func coreDataWillInvokeMigration(storeAt storeFileUrl: URL, for model: NSManagedObjectModel) -> Bool {
    ...
    private func requiresPostMigrationStep(storeAt storeFileUrl: URL) -> Bool {
        do {
            /*
            ...
            guard let hashDigest = storeMetadata["NSStoreModelVersionHashesDigest"] as? String else {
            ...
            // Nothing to migrate if we even can not read metadata of the persistent store
            Log.error("Migration detector failed to read persistent store metadata: " + error.localizedDescription, domain: .storage)
            return false
        }
    }
