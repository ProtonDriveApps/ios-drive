...
class LegacyDownloadFileOperation: SynchronousOperation, DownloadOperation {
    ...
    override func start() {
        ...
        self.cloudSlot.scanNode(fileIdentifier, linkProcessingErrorTransformer: { $1 }, handler: { [fileIdentifier] resultFile in
            ...
            case .failure(let error):
                Log
                    .error(
                        "DownloadFileOperation.start could not fetch file details",
                        error: error,
                        domain: .downloader,
            ...
            case .success(let node):
                guard let file = node as? File, let revision = file.activeRevision else {
                    Log.error("DownloadFileOperation.start invalid state", error: Errors.errorReadingMetadata, domain: .downloader, context: LogContext("File: \(fileIdentifier)"))
                ...
                self.cloudSlot.scanRevision(revision.identifier) { [revisionIdentifier = revision.identifier] resultRevision in
                    ...
                    case .failure(let error):
                        Log.error("DownloadFileOperation.start could not scan revision", error: Errors.errorReadingMetadata, domain: .downloader, context: LogContext("Revision: \(revisionIdentifier)"))
                        self.completion?(.failure(error))
                        self.cancel()
                    ...
                    case .success(let updatedRevision):
                        ...
                        guard !blocks.isEmpty else {
                            Log.error("DownloadFileOperation.start, revision does not contain DownloadBlocks", error: Errors.blockListNotAvailable, domain: .downloader, context: LogContext("Revision: \(revisionIdentifier)"))
                        ...
                        let finishOperation = BlockOperation { [weak self] in
                            ...
                            guard let moc = updatedRevision.managedObjectContext else {
                                ...
                                Log
                                    .error(
                                        "DownloadFileOperation.start, revision does not contain DownloadBlocks",
                                        error: error,
                                        domain: .downloader,
                                        context: LogContext("Revision: \(revisionIdentifier)")
                                    )
                                self.completion?(.failure(error))
                                self.cancel()
                                return
                            ...
                            moc.performAndWait {
                                do {
                                    ...
                                    Log
                                        .error(
                                            "DownloadFileOperation.start, failed saving to coredata",
                                            error: error,
                                            domain: .downloader,
                                            context: LogContext("Revision: \(revisionIdentifier)")
                                        )
    ...
    private func createOperationFor(_ block: DownloadBlock) -> DownloadBlockOperation {
        DownloadBlockOperation(downloadTaskURL: URL(string: block.downloadUrl)!, endpointFactory: endpointFactory) { [weak self] result in
            ...
            case .success(let intermediateUrl):
                ...
                block.managedObjectContext?.performAndWait { [fileIdentifier] in
                    do {
                        ...
                        Log
                            .error(
                                "DownloadFileOperation.createDownloaded storage",
                                error: error,
                                domain: .downloader,
                                context: LogContext("File: \(fileIdentifier)")
                            )
            ...
            case .failure(let error):
                Log
                    .error(
                        "DownloadFileOperation.createDownloaded error",
                        error: error,
                        domain: .downloader,
                        context: LogContext("File: \(fileIdentifier)")
                    )
    ...
    private func createEmptyFile(in updatedRevision: Revision) {
        ...
        moc.performAndWait {
            do {
                ...
                Log.error("Creating empty file failed", error: error, domain: .storage)
                self.completion?(.failure(error))
                self.cancel()
            }
        }
