...
final class GalleryCoordinator: PhotosPermissionsCoordinator, PhotoItemCoordinator, PhotosActionCoordinator, PhotosStorageCoordinator, PhotosStateCoordinator, PhotoUpsellCoordinator {
    ...
    private lazy var groupToAlbumController: GroupToAlbumSheetPresenterProtocol = {
        GroupToAlbumFactory().makePresenter(container: container, rootViewController: rootViewController)
    }()
    ...
    init(container: GallerySceneContainer) {
    ...
    func openPreview(id: PhotoId, albumId: AlbumIdentifier?) {
        let viewController = container.makePreviewController(id: id, albumId: albumId)
