...
final class PlayerDataSource: ObservableObject {
    ...
    @Published private(set) var didPlayToEnd: Bool = false
    ...
    func addObservers() {
        ...
        let rateObserver = player.observe(\.rate) { [weak self] player, _ in
            if player.rate > 0 { self?.didPlayToEnd = false }
        }
        observers = [statusObserver, boundsObserver, rateObserver]
    }
