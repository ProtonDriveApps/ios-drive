...
final class LivePhotoPreviewView: UIView, PreviewBadgeSupport {
    ...
    init(photoURL: URL, videoURL: URL?, isLoading: Bool) {
        ...
        // Set still photo immediatelly to avoid flashing issue caused by asynchronicity of live photo loading
        setStillPhoto()
        // Load live photo if possible and replace still version when it's available
        setLivePhotoIfPossible()
    }
...
extension LivePhotoPreviewView {
    private func setStillPhoto() {
        guard let data = try? Data(contentsOf: photoURL) else {
            return
        }
        setupPlaceholder(data: data)
    }

    private func setLivePhotoIfPossible() {
        guard let videoURL else {
            // Dispatch async to make sure frame of placeholderView is correct
        ...
        Task {
            ...
            let resources = [photoURL, videoURL]
            for await livePhoto in PHLivePhoto.load(resources: resources, placeholderImage: placeholderImage) {
    ...
    private func setupPlaceholder(data: Data) {
