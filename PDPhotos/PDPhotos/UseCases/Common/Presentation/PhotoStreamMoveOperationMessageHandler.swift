...
struct PhotoStreamMoveOperationMessageHandler: PhotosMoveOperationMessageHandlerProtocol {
    ...
    func showAddPhotosBanner(for result: PhotosMoveOperationResult) {
        var messages: [String] = []
        if result.failure != 0 {
        ...
        if result.success != 0 {
            let message = Localization.item_add_to_photo_stream(num: result.success)
            messages.append(message)
        }
    ...
    func showRemovePhotosBanner(for result: PhotosMoveOperationResult) {
        var messages: [String] = []
        if result.failure != 0 {
        ...
        if result.success != 0 {
            let message = Localization.item_remove_from_photo_stream(num: result.success)
            messages.append(message)
        }
        if messages.isEmpty { return }
        let message = messages.joined(separator: ", ")
