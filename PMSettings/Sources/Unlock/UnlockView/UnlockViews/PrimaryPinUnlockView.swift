...
final class PrimaryPinUnlockView: UIView {
    ...
    init(viewModel: PinUnlockViewModel) {
        ...
        self.viewModel.onPinUnlockFailure = { [weak self] in
            var errorMessage = "Wrong PIN."
            if let numberOfAttemptsLeft = viewModel.numberOfAttemptsLeft {
                errorMessage += " Attempts left: \(numberOfAttemptsLeft)"
            }
            self?.textField.setTemporaryError(errorMessage)
