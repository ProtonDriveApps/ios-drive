...
protocol PinUnlockViewModel {
    ...
    var numberOfAttemptsLeft: Int? { get }
...
public class UnlockViewModel: BiometricUnlockViewModel, PinUnlockViewModel {
    ...
    let failedAttemptsCounter: FailedAttemptsCounter?
    ...
    public init(bioUnlocker: BioUnlocker,
        ...
        self.failedAttemptsCounter = failedAttemptsCounter
    ...
    public var numberOfAttemptsLeft: Int? {
        failedAttemptsCounter?.attemptsLeft
    }
    ...
    public func signOut() {
        logoutManager?.logout { [weak self] result in
            switch result {
            case .success:
                self?.failedAttemptsCounter?.numberOfFailedAttempts = 0
    ...
    public func onFailedNumberOfAttemptsExceeded() {
        signOut()
    }
    ...
    public func unlockWithPin(_ pin: String) {
        pinUnlocker.pinUnlock(pin: pin) { [weak self] success in
            if success {
                ...
                self?.failedAttemptsCounter?.numberOfFailedAttempts += 1
                self?.onPinUnlockFailure?()
                
                if self?.failedAttemptsCounter?.exceededNumberOfAttempts == true {
                    self?.onFailedNumberOfAttemptsExceeded()
                }
            }
        }
    }
