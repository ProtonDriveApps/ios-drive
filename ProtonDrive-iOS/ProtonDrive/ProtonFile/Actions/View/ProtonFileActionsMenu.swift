...
final class ProtonFileActionsMenu: UIMenu {
    convenience init(viewModel: ProtonFileActionsViewModelProtocol) {
        let sections = viewModel.sections.map { section in
            let actions = section.map { action in
                ...
                actionButton.accessibilityIdentifier = action.accessibilityLabel
                return actionButton
            }
...
private extension ProtonFileAction {
    var image: UIImage? {
        switch self.type {
        case .openInBrowser:
