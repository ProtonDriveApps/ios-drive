...
@MainActor
final class IIncomingFilesReviewViewModel: ObservableObject {
    ...
    @Published private(set) var selectedFolder: NodeDTO
    ...
    /// Invoked when the user taps the destination row. The host should
    /// present the picker and call the supplied closure with the folder the
    /// user chose.
    var onPickDestination: ((@escaping (NodeDTO) -> Void) -> Void)?
    ...
    var currentDestinationName: String {
        selectedFolder.isRoot ? Localization.menu_text_my_files : selectedFolder.name
    }
    ...
    func requestPickDestination() {
        guard commitEditingIfNeeded() else { return }
        onPickDestination? { [weak self] folder in
...
// MARK: - Save
extension IIncomingFilesReviewViewModel {
    func save(onDismiss: @escaping () -> Void) {
        ...
        Task {
            do {
                try await save(to: destination)
            } catch {
                Log.error("Save incoming files failed", error: error, domain: .shareExtension)
            }

            let deeplink = await makeDeepLink(for: destination) ?? Deeplink()
            await MainActor.run {
    ...
    private func save(to destination: NodeDTO) async throws {
        let copiedURLs = moveSharedFiles()
        let importedFiles = try importUrls(copiedURLs, to: destination)
        upload(files: importedFiles, to: destination)
    }
    ...
    private func importUrls(_ urls: [URL], to destination: NodeDTO) throws -> [CoreDataFile] {
        // `fileImporter` uses the folder's managed object context to perform
        // the import synchronously, so resolve the destination on the main
        // context where the importer's writes are observable.
        let context = tower.storage.mainContext
        guard let folder: CoreDataFolder = try? context.typedObject(with: destination.objectID) else {
            throw CoreDataFolder.InvalidState(message: "Can't find target folder for import")
        }
        var importedFiles: [CoreDataFile] = []
        for url in urls {
            let newFile = try tower.fileImporter.importFile(from: url, to: folder, with: nil)
            importedFiles.append(newFile)
        }
        return importedFiles
    }

    private func upload(files: [CoreDataFile], to destination: NodeDTO) {
    ...
    private func makeDeepLink(for destination: NodeDTO) async -> Deeplink? {
        let context = tower.storage.mainContext
        let objectID = destination.objectID
