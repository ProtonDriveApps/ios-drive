...
struct IncomingFilesReviewView: View {
    @Environment(\.dismiss) private var dismiss
    @StateObject private var viewModel: IncomingFilesReviewViewModel
    ...
    init(
        ...
        _viewModel = StateObject(wrappedValue: IncomingFilesReviewViewModel(
    ...
    var body: some View {
        ...
        .sheet(isPresented: $viewModel.isShowingDestinationPicker) {
            IncomingFilesDestinationPickerScreen(
                nodeID: viewModel.rootNodeID,
                authenticatedContainer: viewModel.authenticatedContainer
            ) { selectedFolder in
                viewModel.update(selectedFolder: selectedFolder)
            }
        }
    }
    ...
    private var destinationRow: some View {
        VStack(spacing: 0) {
            Divider()
            Button {
                guard viewModel.commitEditingIfNeeded() else { return }
                viewModel.isShowingDestinationPicker = true
            } label: {
...
private struct IncomingFilesDestinationPickerScreen: View {
    let nodeID: NodeIdentifier
    let authenticatedContainer: AuthenticatedDependencyContainer
    @StateObject private var coordinator: FinderCoordinator

    init(
        nodeID: NodeIdentifier,
        authenticatedContainer: AuthenticatedDependencyContainer,
        onSaveHere: @escaping (Folder) -> Void
    ) {
        self.nodeID = nodeID
        self.authenticatedContainer = authenticatedContainer
        _coordinator = StateObject(
            wrappedValue: FinderCoordinator(
                container: authenticatedContainer,
                incomingFilesSelectionHandler: onSaveHere
            )
        )
    }
    ...
    var body: some View {
        RootView(vm: RootViewModel()) {
            IncomingFilesDestinationPickerView(nodeID: nodeID, coordinator: coordinator)
        }
    }
}
